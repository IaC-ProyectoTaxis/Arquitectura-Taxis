...
resource "aws_api_gateway_integration" "lambda_post_registro_taxi" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.registro.id
  http_method = aws_api_gateway_method.post_registro.http_method
  ...
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.taxis.invoke_arn
}

resource "aws_api_gateway_integration" "lambda_post_registro_viaje" {
  rest_api_id = aws_api_gateway_rest_api.api.id
  resource_id = aws_api_gateway_resource.registro.id
  http_method = aws_api_gateway_method.post_registro.http_method
  ...
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.viajes.invoke_arn
}
...
resource "aws_lambda_permission" "api_gw_invoke_lambda_taxi" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
...
resource "aws_lambda_permission" "api_gw_invoke_lambda_viaje" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
...
resource "aws_api_gateway_integration_response" "post_integration_response" {
  ...
  depends_on = [
    ...
    aws_api_gateway_integration.lambda_post_registro_taxi,
    aws_api_gateway_integration.lambda_post_registro_viaje
  ]
}